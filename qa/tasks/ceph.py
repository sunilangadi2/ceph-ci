...
def cluster(ctx, config):
    ...
    try:
        ...
        if first_in_kern_log('BUG:|Oops:|INFO: task .+ blocked for more than .+ seconds|KASAN',
                             config['log_ignorelist']) is not None:
            log.warning('Found errors (BUG|Oops|blocked|KASAN) in kernel log')
            ctx.summary['success'] = False
            # use the most severe problem as the failure reason
            if 'failure_reason' not in ctx.summary:
                for pattern in ['Oops:', 'BUG:', 'INFO: task .+ blocked for more than .+ seconds.', 'KASAN']:
                    match = first_in_ceph_log(pattern, config['log_ignorelist'])
                    if match is not None:
                        ctx.summary['failure_reason'] = \
