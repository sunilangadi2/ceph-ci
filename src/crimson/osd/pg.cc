  ...
  const OpInfo &op_info)
{
  ...
  return interruptor::do_for_each(
	...
	logger().debug(
	  ...
	  *m,
      ...
      [this, m, &op_info] (auto&& txn,
			   auto&& obc,
			   auto&& osd_op_p,
                           bool user_modify) -> interruptible_osd_op_errorator::future<> {
	logger().debug(
    ...
    [ox_deleter = std::move(ox), m, obc, this] (const crimson::osd::error& e) {
...
PG::with_clone_obc(hobject_t oid, with_obc_func_t&& func)
{
  ...
  return with_head_obc<RWState::RWREAD>(oid.get_head(),
    [oid, func=std::move(func), this](auto head) -> interruptible_load_obc_ertr::future<> {
    ...
    return clone->template with_lock<State>(
      [coid=*coid, existed=existed,
       ...
       func=std::move(func), this]() -> interruptible_load_obc_ertr::future<> {
      auto loaded = interruptible_load_obc_ertr::make_ready_future<ObjectContextRef>(clone);
      if (existed) {
...
template PG::interruptible_load_obc_ertr::future<>
