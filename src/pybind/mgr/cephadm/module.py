...
> ssh -F ssh_config -i ~/cephadm_private_key {self.ssh_user}@{addr}'''
    ...
    def remove_host(self, host: str, force: bool = False, offline: bool = False) -> str:
        ...
        host_offline = host in self.offline_hosts or self.inventory._inventory[host].get(
            'status', '').lower() == 'offline'

        if host_offline and not offline:
            return "{} is offline, please use --offline and --force to remove this host.".format(host)
