...
3. Run a simple 4KiB random write workload on an OSD using the following
  ...
  .. note:: Note that before running the test, caches must be dropped to get an
...
5. Note the overall throughput(IOPS) obtained from output. This value is the
   baseline throughput(IOPS) when the default bluestore throttle options are
   in effect.
6. If the intent is to determine the bluestore throttle values for your
...
7. Run the 4KiB random write test as before using OSD bench.
8. Note the overall throughput from the output and compare the value
   against the baseline throughput recorded in step 5.
9. If the throughput doesn't match with the baseline, increment the bluestore
   throttle options by 2x and repeat steps 7 through 9 until the obtained
