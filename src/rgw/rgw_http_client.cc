...
int RGWHTTPClient::init_request(rgw_http_req_data *_req_data)
{
  ...
  if (!verify_ssl) {
    ...
    dout(0) << "pacific : ssl verification is set to off" << dendl;
  } else {
    if (!ca_path.empty()) {
      curl_easy_setopt(easy_handle, CURLOPT_CAINFO, ca_path.c_str());
      dout(0) << "pacfic : using custom ca cert "<< ca_path.c_str() << " for ssl" << dendl;
    }
	...
	dout(0) << "pacific using custom client cert " << client_cert.c_str()
	  << " and private key " << client_key.c_str() << dendl;
      } else {
	dout(0) << "pacific private key is missing for client certificate" << dendl;
      }
    }
  }
