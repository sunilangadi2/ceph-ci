...
int BlockCrypto<T>::crypt(ceph::bufferlist* data, uint64_t image_offset,
  ...
  ldout(m_cct, 20) << "encrypt data in crypt function: \n";
  data->hexdump(*_dout);
  *_dout << dendl;
  ...
  for (auto buf = src.buffers().begin(); buf != src.buffers().end(); ++buf) {
    ...
    while (remaining_buf_bytes > 0) {
      ...
      ldout(m_cct, 20) << " crypt data in_buf: " << in_buf_ptr << dendl;
      ldout(m_cct, 20) << " crypt data out_buf: " << out_buf_ptr << dendl;
    }
  }
