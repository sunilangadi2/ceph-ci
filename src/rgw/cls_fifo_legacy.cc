...
struct Updater {
  ...
  lr::AioCompletion* cur = lr::Rados::aio_create_completion(
    static_cast<void*>(this), &FIFO::update_callback, nullptr);
  fifo::update update;
  fifo::objv version;
  bool reread = false;
...
void FIFO::update_callback(lr::completion_t, void* arg)
{
  ...
  if (!updater->reread) {
	...
	arg, &FIFO::update_callback, nullptr);
      updater->reread = true;
...
struct Reader {
  ...
  std::uint64_t tid;
  lr::AioCompletion* cur = lr::Rados::aio_create_completion(
    static_cast<void*>(this), &FIFO::read_callback, nullptr);
...
struct Trimmer {
  ...
  std::uint64_t tid;
  lr::AioCompletion* cur = lr::Rados::aio_create_completion(
    static_cast<void*>(this), &FIFO::trim_callback, nullptr);
  bool update = false;
  bool canceled = false;
  int retries = 0;
...
void FIFO::trim_callback(lr::completion_t, void* arg)
{
  ...
  if (!trimmer->update) {
    ...
    if (trimmer->pn < trimmer->part_num) {
      ...
      l.unlock();
      trimmer->cur->release();
      trimmer->cur = lr::Rados::aio_create_completion(arg, &FIFO::trim_callback, nullptr);
    ...
    r = trimmer->fifo->trim_part(trimmer->part_num, trimmer->ofs,
				 std::nullopt, trimmer->exclusive, tid,
				 trimmer->cur);
    if (r < 0) {
      lderr(cct) << __PRETTY_FUNCTION__ << ":" << __LINE__
		 << " failed scheduling trim: r=" << r << " tid=" << tid
		 << dendl;
      complete(trimmer->super, r);
    } else {
      trimmer.release();
