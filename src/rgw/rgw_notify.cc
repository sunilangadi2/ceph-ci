...
public:
  ...
  Manager(CephContext* _cct, uint32_t _max_queue_size, uint32_t _queues_update_period_ms, 
          uint32_t _queues_update_retry_ms, uint32_t _queue_idle_sleep_us, uint32_t failover_time_ms, uint32_t _worker_count, RGWRados* store) : 
    ...
    cct(_cct),
    rados_ioctx(store->get_notif_pool_ctx()),
...
bool init(CephContext* cct, RGWRados* store) {
  if (s_manager) {
    return false;
  }
...
void populate_record_from_request(const req_state *s, 
  ...
  record.bucket_id = s->bucket.get_key();
  // pass meta data
...
bool match(const rgw_pubsub_topic_filter& filter, const req_state* s, const rgw_obj_key& obj, EventType event) {
  if (!::match(filter.events, event)) { 
    return false;
  }
  if (!::match(filter.s3_filter.key_filter, obj.name)) {
    return false;
  }
...
int publish_reserve(EventType event_type,
      reservation_t& res) 
{
  RGWUserPubSub ps_user(res.store, res.s->user->user_id);
  ...
  for (const auto& bucket_topic : bucket_topics.topics) {
    ...
    ldout(res.s->cct, 20) << "INFO: notification: '" << topic_filter.s3_id << 
        ...
        "' and bucket: '" << res.s->bucket.get_key() << 
    ...
    if (topic_cfg.dest.persistent) {
      ...
      auto ret = rgw_rados_operate(res.store->get_notif_pool_ctx(), 
...
int publish_commit(const rgw_obj_key& obj,
        uint64_t size,
        const ceph::real_time& mtime, 
        const std::string& etag, 
        EventType event_type,
  ...
  for (auto& topic : res.topics) {
    ...
    if (topic.cfg.dest.persistent) { 
      ...
      if (bl.length() > res.size) {
        ...
        ret = rgw_rados_operate(res.store->get_notif_pool_ctx(), 
          queue_name, &op, librados::OPERATION_RETURNVEC, res.s->yield);
        if (ret < 0) {
      ...
      const auto ret = rgw_rados_operate(res.store->get_notif_pool_ctx(),
            queue_name, &op,
            res.s->yield);
