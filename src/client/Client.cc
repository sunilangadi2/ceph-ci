...
int Client::fchmodat(int dirfd, const char *relpath, mode_t mode, int flags,
                     const UserPerm& perms) {
  RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
  if (!mref_reader.is_state_satisfied()) {
    return -CEPHFS_ENOTCONN;
...
int Client::fchownat(int dirfd, const char *relpath, uid_t new_uid, gid_t new_gid,
...
int Client::fstatxat(int dirfd, const char *relpath,
