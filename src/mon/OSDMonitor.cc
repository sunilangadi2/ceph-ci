...
bool OSDMonitor::preprocess_command(MonOpRequestRef op)
{
  ...
  string format = cmd_getval<string>(cmdmap, "format", "plain");
  ...
  if (prefix == "osd stat") {
    ...
    epoch_t epoch = cmd_getval<int64_t>(cmdmap, "epoch", osdmap.get_epoch());
				      ...
				      const cmdmap_t& cmdmap)
{
  ...
  if (prefix == "osd setcrushmap" ||
	...
	// default one hour
	double d = cmd_getval<double>(cmdmap, "expire",
    ...
    int64_t pg_num = cmd_getval<int64_t>(cmdmap, "pg_num", 0);
    int64_t pg_num_min = cmd_getval<int64_t>(cmdmap, "pg_num_min", 0);
    int64_t pgp_num = cmd_getval<int64_t>(cmdmap, "pgp_num", pg_num);
	...
	cmd_getval<int64_t>(cmdmap, "expected_num_objects", 0);
    } else {
    ...
    int64_t fast_read_param = cmd_getval<int64_t>(cmdmap, "fast_read", -1);
