...
class PSTopicS3_V3:
    ...
    POST ?Action=DeleteTopic&TopicArn=<topic-arn>
    """
    def __init__(self, conn, host, port, topic_name, region, endpoint_args=None, opaque_data=None):
        self.conn = conn
        self.host = host
        self.port = port
        self.topic_name = topic_name.strip()
        ...
        if opaque_data is not None:
            self.attributes['OpaqueData'] = opaque_data
        protocol = 'https' if conn.use_ssl else 'http'
        self.client = boto3.client('sns',
                           endpoint_url=protocol+'://'+host+':'+str(port),
                           aws_access_key_id=conn.aws_access_key_id,
                           aws_secret_access_key=conn.aws_secret_access_key,
                           region_name=region,
                           verify='./cert.pem')
    ...
    def get_config(self):
        ...
        headers = {'Authorization': 'AWS '+self.conn.aws_access_key_id+':'+signature,
                   ...
                   'Content-Type': content_type}
        if self.conn.use_ssl:
            http_conn = http_client.HTTPSConnection(self.host, self.port,
                    context=ssl.create_default_context(cafile='./cert.pem'))
        else:
            http_conn = http_client.HTTPConnection(self.host, self.port)
        http_conn.request(method, resource, body, headers)
        response = http_conn.getresponse()
        data = response.read()
        status = response.status
...
class PSNotificationS3_V3:
    """class to set/get/delete an S3 notification
    PUT /<bucket>?notification
    GET /<bucket>?notification[=<notification>]
    DELETE /<bucket>?notification[=<notification>]
    """
    def __init__(self, conn, bucket_name, topic_conf_list, host, port):
        self.conn = conn
        self.host = host
        self.port = port
        assert bucket_name.strip()
