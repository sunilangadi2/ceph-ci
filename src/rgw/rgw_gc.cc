...
static const string gc_oid_prefix = "gc";
static const string gc_index_lock_name = "gc_process";
...
int RGWGC::set_to_transitioned(int index)
{
  ObjectWriteOperation op;
  obj_version objv;
  objv.ver = static_cast<uint64_t>(GC_OBJ_STATE::READY_FOR_TRANSITION);
  cls_version_check(op, objv, VER_COND_EQ);
  objv.ver = static_cast<uint64_t>(GC_OBJ_STATE::TRANSITIONED_TO_Q);
...
int RGWGC::list(int *index, string& marker, uint32_t max, bool expired_only, std::list<cls_rgw_gc_obj_info>& result, bool *truncated, bool& processing_queue)
{
  ...
  for (; *index < max_objs && result.size() < max; (*index)++, marker.clear(), check_queue = false) {
    ...
    if (objv.ver != static_cast<uint64_t>(GC_OBJ_STATE::TRANSITIONED_TO_Q) && ! check_queue && ! processing_queue) {
      ...
      if ((objv.ver == static_cast<uint64_t>(GC_OBJ_STATE::READY_FOR_TRANSITION)) && (entries.size() < max - result.size())) {
        check_queue = true;
        marker.clear();
      }
    }
                   ...
                   RGWGCIOManager& io_manager)
{
  ...
  do {
    ...
    if (! transitioned_objects_cache[index]) {
      ...
      obj_version objv;
      cls_version_read(store->gc_pool_ctx, obj_names[index], &objv);
      if ((objv.ver == static_cast<uint64_t>(GC_OBJ_STATE::READY_FOR_TRANSITION)) && entries.size() == 0) {
        std::list<cls_rgw_gc_obj_info> non_expired_entries;
        ret = cls_rgw_gc_list(store->gc_pool_ctx, obj_names[index], marker, 1, false, non_expired_entries, &truncated, next_marker);
        if (non_expired_entries.size() == 0) {
          ...
          set_to_transitioned(index);
          marker.clear();
          ldpp_dout(this, 20) << "RGWGC::process cls_rgw_gc_list returned NO non expired entries, so setting cache entry to TRUE for index" << index << dendl;
        } else {
          ret = 0;
          goto done;
        }
      }
      if ((objv.ver == static_cast<uint64_t>(GC_OBJ_STATE::NOT_READY_FOR_TRANSITION)) && (ret == -ENOENT || entries.size() == 0)) {
        ret = 0;
        goto done;
      }
    }
