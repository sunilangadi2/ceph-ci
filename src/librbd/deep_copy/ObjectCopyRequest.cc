...
void ObjectCopyRequest<I>::process_copyup() {
  if (m_snapshot_sparse_bufferlist.empty()) {
    bool object_exists = (m_src_snap_id_start > 0 && m_snapshot_delta.empty());

    // no data to copy or truncate/zero
    finish(object_exists ? 0 : -ENOENT);
    return;
  }
...
void ObjectCopyRequest<I>::compute_zero_ops() {
  ...
  // exists if copying from an arbitrary snapshot w/ an empty snapshot delta
  bool object_exists = (m_src_snap_id_start > 0 && m_snapshot_delta.empty());
