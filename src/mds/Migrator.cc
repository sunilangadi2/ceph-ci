...
void Migrator::handle_mds_failure_or_stop(mds_rank_t who)
{
  ...
  while (q != import_state.end()) {
	...
	dout(10) << "import state=prepped : unpinning base+bounds, unfreezing " << *dir << dendl;
	{
	  ...
	  if (unlikely(g_conf()->mds_kill_import_at == 10)) {
	    ceph_abort();
	  }
	}
	break;
...
void Migrator::dispatch_export_dir(MDRequestRef& mdr, int count)
{
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 1)) {
    ceph_abort();
  }
  ...
  if (results.size() == 1 && results.front().first == dir) {
    ...
    if (unlikely(g_conf()->mds_kill_export_at == 2)) {
      ceph_abort();
    }
...
void Migrator::handle_export_discover_ack(const cref_t<MExportDirDiscoverAck> &m)
{
  ...
  if (it == export_state.end() ||
    ...
    if (m->is_success()) {
      ...
      if (unlikely(g_conf()->mds_kill_export_at == 3)) {
	ceph_abort();
      }
...
void Migrator::export_frozen(CDir *dir, uint64_t tid)
{
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 4)) {
    ceph_abort();
  }
...
void Migrator::handle_export_prep_ack(const cref_t<MExportDirPrepAck> &m)
{
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 5)) {
    ceph_abort();
  }
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 6)) {
    ceph_abort();
  }
...
void Migrator::export_go_synced(CDir *dir, uint64_t tid)
{
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 7)) {
    ceph_abort();
  }
...
void Migrator::handle_export_ack(const cref_t<MExportDirAck> &m)
{
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 9)) {
    ceph_abort();
  }
  ...
  mds->mdlog->flush();
  if (unlikely(g_conf()->mds_kill_export_at == 10)) {
    ceph_abort();
  }
}
...
void Migrator::export_logged_finish(CDir *dir)
{
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 11)) {
    ceph_abort();
  }
...
void Migrator::export_finish(CDir *dir)
{
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 12)) {
    ceph_abort();
  }
  ...
  if (unlikely(g_conf()->mds_kill_export_at == 13)) {
    ceph_abort();
  }
...
void Migrator::handle_export_discover(const cref_t<MExportDirDiscover> &m, bool started)
{
  ...
  if (unlikely(g_conf()->mds_kill_import_at == 1)) {
    ceph_abort();
  }
  ...
  if (unlikely(g_conf()->mds_kill_import_at == 2)) {
    ceph_abort();
  }
}
...
void Migrator::handle_export_prep(const cref_t<MExportDirPrep> &m, bool did_assim)
{
  ...
  if (!did_assim) {
    ...
    if (unlikely(g_conf()->mds_kill_import_at == 3)) {
      ceph_abort();
    }
  ...
  if (unlikely(g_conf()->mds_kill_import_at == 4)) {
    ceph_abort();
  }
}
...
{
  if (unlikely(g_conf()->mds_kill_import_at == 5)) {
    ceph_abort();
  }
  ...
  if (unlikely(g_conf()->mds_kill_import_at == 6)) {
    ceph_abort();
  }
				   ...
				   map<client_t,pair<Session*,uint64_t> >& imported_session_map)
{
  ...
  if (unlikely(g_conf()->mds_kill_import_at == 7)) {
    ceph_abort();
  }
  ...
  if (unlikely(g_conf()->mds_kill_import_at == 8)) {
    ceph_abort();
  }
...
void Migrator::import_finish(CDir *dir, bool notify, bool last)
{
  ...
  // log finish
  if (unlikely(g_conf()->mds_kill_import_at == 9)) {
    ceph_abort();
  }
