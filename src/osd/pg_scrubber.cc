...
void PgScrubber::on_digest_updates()
{
  ...
  if (m_end.is_max()) {
    ...
    //  be dispatched?
    //  A possible problem is a 2'nd DigestUpdate event. We might fail in various
    //  ways, including in the assert.
    // Does it merit a new state? a new state creates a sync problem, as well.
    // )
