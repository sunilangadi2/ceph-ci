    ...
    try {
      if (bfs::exists(cache_location)) {
        ...
        if (g_conf()->rgw_d3n_l1_evict_cache_on_start) {
          ...
          for (auto& p : bfs::directory_iterator(cache_location)) {
            bfs::remove_all(p.path());
          }
        }
      } else {
        ...
        bfs::create_directories(cache_location);
      }
