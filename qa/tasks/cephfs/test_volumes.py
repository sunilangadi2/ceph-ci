...
class TestVolumes(CephFSTestCase):
    ...
    def test_subvolume_authorized_list(self):
        ...
        # json doesn't keep the order, hence comparing each item
        self.assertEqual(len(expected_auth_list), len(auth_list))
        for i in range(3):
            self.assertIn(expected_auth_list[i], auth_list)
