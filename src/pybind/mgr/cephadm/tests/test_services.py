...
from ceph.deployment.service_spec import IscsiServiceSpec, GrafanaSpec
from cephadm.tests.fixtures import _run_cephadm, with_host, with_service
...
class TestGrafanaService:
    ...
    def testgenerate_config(self, cephadm_module: CephadmOrchestrator):
                ...
                assert out == (
                    {
                        'files':
                            {
                                'certs/cert_file': mock.ANY,
                                'certs/cert_key': mock.ANY,
                                'grafana.ini':
