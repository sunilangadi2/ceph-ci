...
TEST(LibCephFS, Openat) {
  ...
  struct ceph_statx stx;
  ASSERT_EQ(ceph_fstatxat(cmount, root_fd, c_rel_dir, &stx, 0, 0), 0);
  ...
  ASSERT_EQ(ceph_fstatxat(cmount, dir_fd, c_rel_path, &stx, 0, 0), 0);
...
TEST(LibCephFS, Fstatxat) {
  ...
  ASSERT_LE(0, fd);
  ASSERT_EQ(ceph_fstatxat(cmount, fd, dir_name, &stx, 0, 0), 0);
  ASSERT_EQ(stx.stx_mode & S_IFMT, S_IFDIR);
  ASSERT_EQ(ceph_fstatxat(cmount, fd, rel_file_name_2, &stx, 0, 0), 0);
  ASSERT_EQ(stx.stx_mode & S_IFMT, S_IFREG);
  ASSERT_EQ(0, ceph_close(cmount, fd));
  ...
  ASSERT_LE(0, fd);
  ASSERT_EQ(ceph_fstatxat(cmount, fd, rel_file_name_1, &stx, 0, 0), 0);
  ASSERT_EQ(stx.stx_mode & S_IFMT, S_IFREG);
  ...
  ASSERT_EQ(ceph_fstatxat(cmount, fd, rel_file_name_1, &stx, 0, 0), -ENOENT);
  ASSERT_EQ(0, ceph_close(cmount, fd));
...
TEST(LibCephFS, FstatxatATFDCWD) {
  struct ceph_mount_info *cmount;
  ASSERT_EQ(ceph_create(&cmount, NULL), 0);
  ASSERT_EQ(ceph_conf_read_file(cmount, NULL), 0);
  ASSERT_EQ(0, ceph_conf_parse_env(cmount, NULL));
  ...
  ASSERT_EQ(ceph_fstatxat(cmount, CEPHFS_AT_FDCWD, rel_file_name_1, &stx, 0, 0), 0);
  ASSERT_EQ(stx.stx_mode & S_IFMT, S_IFREG);
...
TEST(LibCephFS, Fchownat) {
  ...
  // change ownership to nobody -- we assume nobody exists and id is always 65534
  ASSERT_EQ(ceph_conf_set(cmount, "client_permissions", "0"), 0);
  ASSERT_EQ(ceph_fchownat(cmount, fd, rel_file_path, 65534, 65534, 0), 0);
  ASSERT_EQ(ceph_conf_set(cmount, "client_permissions", "1"), 0);
  ceph_close(cmount, fd);
...
TEST(LibCephFS, FchownatATFDCWD) {
  pid_t mypid = getpid();
  ...
  // change ownership to nobody -- we assume nobody exists and id is always 65534
  ASSERT_EQ(ceph_conf_set(cmount, "client_permissions", "0"), 0);
  ASSERT_EQ(ceph_fchownat(cmount, CEPHFS_AT_FDCWD, rel_file_path, 65534, 65534, 0), 0);
  ASSERT_EQ(ceph_conf_set(cmount, "client_permissions", "1"), 0);
...
TEST(LibCephFS, Fchmodat) {
  ...
  // set perms to read but can't write
  ASSERT_EQ(ceph_fchmodat(cmount, fd, rel_file_path, 0400, 0), 0);
  ASSERT_EQ(ceph_open(cmount, file_path, O_RDWR, 0), -EACCES);

  // reset back to writeable
  ASSERT_EQ(ceph_fchmodat(cmount, fd, rel_file_path, 0600, 0), 0);
...
TEST(LibCephFS, FchmodatATFDCWD) {
  pid_t mypid = getpid();
  ...
  // set perms to read but can't write
  ASSERT_EQ(0, ceph_chdir(cmount, dir_path));
  ASSERT_EQ(ceph_fchmodat(cmount, CEPHFS_AT_FDCWD, rel_file_path, 0400, 0), 0);
  ASSERT_EQ(ceph_open(cmount, file_path, O_RDWR, 0), -EACCES);

  // reset back to writeable
  ASSERT_EQ(ceph_fchmodat(cmount, CEPHFS_AT_FDCWD, rel_file_path, 0600, 0), 0);
  int fd2 = ceph_open(cmount, file_path, O_RDWR, 0);
  ASSERT_LE(0, fd2);
  ASSERT_EQ(0, ceph_close(cmount, fd2));
