...
class SubvolumeV2(SubvolumeV1):
    ...
    @staticmethod
    def safe_to_remove_subvolume_clone(subvol_state, internal_cleanup):
        ...
        if not internal_cleanup and subvol_state not in acceptable_rm_clone_states:
            return False
        return True
    ...
    def remove(self, retainsnaps=False, internal_cleanup=False):
        if self.list_snapshots():
            ...
            if not self.safe_to_remove_subvolume_clone(self.state, internal_cleanup):
