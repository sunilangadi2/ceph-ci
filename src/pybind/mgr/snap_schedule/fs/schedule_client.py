...
from typing import Any, Callable, Dict, Iterator, List, Sequence, Set, Optional, Tuple, TypeVar, Union
import sqlite3
...
def updates_schedule_db(func: Callable[..., None]) -> Callable[..., None]:
...
class SnapSchedClient(CephfsClient):
    ...
    def store_snap_schedule(self,
                            fs: str, path_: str,
                            args: Tuple[str, str, str, str, str, str]) -> None:
        sched = Schedule(*args)
    ...
    def rm_snap_schedule(self,
                         fs: str, path:
                         str, schedule: str, start: str) -> None:
        db = self.get_schedule_db(fs)
    ...
    @updates_schedule_db
    def activate_snap_schedule(self, fs: str, path: str, schedule: str, start: str) -> None:
        db = self.get_schedule_db(fs)
