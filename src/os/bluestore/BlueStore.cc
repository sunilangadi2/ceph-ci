...
void BlueStore::_zoned_cleaner_thread() {
  ...
  while (true) {
	...
	break;
      ...
      dout(20) << __func__ << " sleep" << dendl;
      zoned_cleaner_in_progress = false;
      zoned_cleaner_cond.wait(l);
      dout(20) << __func__ << " wake" << dendl;
    } else {
      l.unlock();
      while (!zoned_cleaner_queue.empty()) {
	_zoned_clean_zone(zoned_cleaner_queue.front());
	zoned_cleaner_queue.pop_front();
      }
      l.lock();
    }
  }
  ...
  WriteContext *wctx)
{
  ...
  if (bdev->is_smr()) {
    std::lock_guard l{zoned_cleaner_lock};
    if (!zoned_cleaner_in_progress && shared_alloc.a->zoned_needs_cleaning()) {
      zoned_cleaner_queue = shared_alloc.a->zoned_get_zones_to_clean();
      zoned_cleaner_in_progress = true;
