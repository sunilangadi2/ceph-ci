...
enum class global_id_status_t {
  ...
  NEW_PENDING,
  // fresh client; appears to be new enough to correctly reclaim global_id
  NEW_OK,
  // fresh client; unknown whether it can reclaim global_id correctly
  NEW_NOT_EXPOSED,
  // reconnecting client (global_id != 0); waiting for CephXAuthenticate
  RECLAIM_PENDING,
  // reconnecting client; correctly reclaimed global_id
  RECLAIM_OK,
  // reconnecting client; did not properly prove prior global_id ownership
  RECLAIM_UNVERIFIED
};
