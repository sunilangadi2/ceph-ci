...
void RocksDBStore::compact_thread_entry()
{
  ...
  while (!compact_queue_stop ||
	  !compact_queue_clone.empty() || !compact_queue.empty()) {
    if (compact_queue_clone.empty() && !compact_queue.empty()) {
      std::swap(compact_queue_clone, compact_queue);
    }
    if (!compact_queue_clone.empty()) {
      l.unlock();
      auto& [s, e, remove, p] = compact_queue_clone.front();
      ...
      if (s.empty() && e.empty()) {
        ...
        if (!remove) {
          ...
          for (++i; i != compact_queue_clone.end();) {
	      ...
	      ++entries_processed;
              ...
              if (!r1 || p != p1) {
                ...
                dout(20) << __func__ << " rm range "
                  ...
                  << dendl;
                t->rm_range_keys_unconditionally(p1, s1, e1);
                i = compact_queue_clone.erase(i);
		++entries_processed;
      ...
      ++entries_processed;
      logger->inc(l_rocksdb_compact_queue_processed, entries_processed);

      dout(20) << __func__ << " left " << compact_queue_clone.size() << dendl;
      l.lock();
      continue;
    }
