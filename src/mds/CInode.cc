                                 ...
                                 MDSContext *fin)
{
  ...
  public:
	...
	using ceph::decode;
        ...
        // so skip the backtrace scrub for this entry and say that all's well
        if (in->is_dirty_parent()) {
          dout(20) << "decode failed; forcing backtrace as passed since inode is dirty parent" << dendl;
          results->backtrace.passed = true;
        }
	...
	goto next;
      ...
      if (divergent || memory_newer < 0) {
         ...
         */
        if (divergent && in->get_inode()->nlink == 0) {
          results->backtrace.passed = true;
          dout(20) << "divergent bactraces are acceptable when dn is being purged " << *in << dendl;
        }
      } else {
        results->backtrace.passed = true;
      }
...
next:
	...
	goto next;
      ...
      results->raw_stats.passed = true;
      {
        MDCache *mdcache = in->mdcache;
        auto ino = [this]() { return in->ino(); };
        dout(20) << "raw stats check passed" << dendl;
      }
