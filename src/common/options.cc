...
std::vector<Option> get_cephfs_mirror_options() {
  return std::vector<Option>({
    ...
    Option("cephfs_mirror_max_concurrent_failures_per_directory", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_default(10)
    .set_min(0)
    .set_description("concurrent failed directory synchronization attempts before marking a directory as \"failed\"")
    .set_long_description("number of concurrent snapshot synchronization failues to mark a directory as \"failed\". failed directories are retried for synchronization less frequently."),
