...
std::vector<Option> get_global_options() {
  return std::vector<Option>({
    ...
    Option("auth_allow_unverified_global_ids", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_default(true)
    ...
    .set_long_description("Allowing unauthorized global_id (re)use poses a security risk.  Unfortunately, older clients may omit their ticket on reconnects and therefore rely on this being allowed for preserving their global_id for the lifetime of the client instance.  Setting this value to false would immediately prevent new connections from those clients (assuming auth_expose_unverified_global_ids set to true) and eventually break existing sessions as well (regardless of auth_expose_unverified_global_ids setting).")
    .add_see_also("auth_expose_unverified_global_ids"),

    Option("auth_expose_unverified_global_ids", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_default(true)
    ...
    .set_long_description("In permissive mode (auth_allow_unverified_global_ids set to true), this helps with identifying clients that are not patched.  In enforcing mode (auth_allow_unverified_global_ids set to false), this is a fail-fast mechanism: don't establish a session that will almost inevitably be broken later.")
    .add_see_also("auth_allow_unverified_global_ids"),
