...
function rename_files()
{
  ...
  do
    ...
    mv ${temp_dir}/file_${i} ${temp_dir}/rename/
    if [ $? -ne 0 ] ; then
      return 1
    fi
  done
...
function make_symlinks()
{
  ...
  for i in $(seq 1 $1)
  do
    ln -s ../file ${temp_dir}/symlink/sym_${i}
    if [ $? -ne 0 ] ; then
      return 1
    fi
  done
...
function make_links()
{
  ...
  for i in $(seq 1 $1)
  do
    ln ${temp_dir}/file ${temp_dir}/link/link_${i}
    if [ $? -ne 0 ] ; then
      return 1
    fi
  done
