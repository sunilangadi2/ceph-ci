...
void LogMonitor::update_from_paxos(bool *need_bootstrap)
{
  ...
  while (version > summary.version) {
	...
	summary.channel_info[le.channel].second++;
      ...
      for (auto& p : prune_channels_to) {
	dout(20) << __func__ << " channel " << p.first
...
void LogMonitor::log_external(const LogEntry& le)
{
  ...
  if (g_conf()->mon_cluster_log_to_file) {
    ...
    if (fd >= 0) {
      ...
      ss << le << "\n";
      std::string s = ss.str();
      int err = safe_write(fd, s.c_str(), s.size());
      if (err < 0) {
...
{
  for (auto& i : channel_fds) {
    if (i.second >= 0) {
      dout(10) << __func__ << " closing " << i.first << " (" << i.second << ")" << dendl;
      ::close(i.second);
    }
  }
  channel_fds.clear();
...
void LogMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
  ...
  for (auto& p : summary.channel_info) {
    if (p.second.second - p.second.first > g_conf()->mon_log_max) {
      const version_t from = p.second.first;
      const version_t to = p.second.second - g_conf()->mon_log_max;
      dout(10) << __func__ << " pruning channel " << p.first
	       << " " << from << " -> " << to << dendl;
      prune_channels_to[p.first] = to;
      pending_channel_info[p.first].first = to;
      for (version_t v = p.second.first; v < to; ++v) {
	std::string key;
