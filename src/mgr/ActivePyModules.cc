...
namespace {
  ...
  // and reacquire it afterwards, we should enforce following locking policy:
  // 1. do not acquire locks when holding the GIL, use a lock<PyAllowThreads>
  //    to to guard the code which acquires non-gil locks.
  // 2. always hold a GIL when calling python functions, for example, when
  //    constructing a PyFormatter instance.
  // a wrapper that provides a convenient RAII-styple mechinary for acquiring
