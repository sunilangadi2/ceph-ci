...
will start to track new ops received afterwards.";
    ...
    int64_t count = cmd_getval<int64_t>(cmdmap, "count", 1LL << 30);
    int64_t bsize = cmd_getval<int64_t>(cmdmap, "size", 4LL << 20);
    int64_t osize = cmd_getval<int64_t>(cmdmap, "object_size", 0);
    int64_t onum = cmd_getval<int64_t>(cmdmap, "object_num", 0);
				 ...
				 const cmdmap_t& cmdmap, ostream &ss)
{
  ...
  if (command == "setomapval" || command == "rmomapkey" ||
    ...
    int64_t shardid = cmd_getval<int64_t>(cmdmap, "shardid", shard_id_t::NO_SHARD);
  ...
  if (command == "set_recovery_delay") {
    int64_t delay = cmd_getval<int64_t>(cmdmap, "utime", 0);
    ostringstream oss;
    oss << delay;
  ...
  if (command == "injectfull") {
    int64_t count = cmd_getval<int64_t>(cmdmap, "count", -1);
    string type = cmd_getval<string>(cmdmap, "type", "full");
