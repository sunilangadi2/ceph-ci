...
extern "C" int ceph_fstatxat(struct ceph_mount_info *cmount, int dirfd, const char *relpath,
                             struct ceph_statx *stx, unsigned int want, unsigned int flags)
{
  ...
  if (flags & ~CEPH_REQ_FLAG_MASK)
    return -EINVAL;
  return cmount->get_client()->fstatxat(dirfd, relpath, stx, cmount->default_perms,
                                        want, flags);
}
