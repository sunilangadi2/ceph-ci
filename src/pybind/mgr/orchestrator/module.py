...
class OrchestratorCli(OrchestratorClientMixin, MgrModule,
    ...
    def _device_light(self,
        ...
        if enable == self.DeviceLightEnable.on:
            return self.light_on(light_type.name, devid)
        else:
            return self.light_off(light_type.name, devid, force)
...
"""
    ...
    ceph orch daemon add {daemon_type or '<daemon_type>'} <placement>"""
        if inbuf:
            ...
            assert daemon_type
            spec = ServiceSpec(daemon_type.name, placement=spec)
    ...
    def _service_action(self, action: ServiceAction, service_name: str):
        ...
        completion = self.service_action(action.name, service_name)
        self._orchestrator_wait([completion])
        raise_if_exception(completion)
        return HandleCommandResult(stdout=completion.result_str())
    ...
    def _daemon_action(self, action: DaemonAction, name: str):
        ...
        completion = self.daemon_action(action.name, name)
        self._orchestrator_wait([completion])
        raise_if_exception(completion)
        return HandleCommandResult(stdout=completion.result_str())
  ...
  ceph orch apply <service_type> [--placement=<placement_string>] [--unmanaged]
        ...
        if inbuf:
            ...
            assert service_type
            specs = [ServiceSpec(service_type.name, placement=placementspec,
