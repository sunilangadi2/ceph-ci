...
// a watch can expire because of time that elapsed without an activity.
// if do, we need to drop it also from permanently-stored `ObjectState`
// instance. this operation resembles a bit the `_UNWATCH` subob
...
public:
  ...
  seastar::future<std::vector<OSDOp>> fabricate_osd_ops() final;
...
seastar::future<std::vector<OSDOp>>
WatchTimeoutRequest::fabricate_osd_ops()
{
