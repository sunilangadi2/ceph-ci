    ...
    async def _remote_connection(self,
        ...
        if not self.cons.get(host):
            ...
            with self.redirect_log(host, addr):
                try:
                    conn = await asyncssh.connect(addr, username=self.mgr.ssh_user, client_keys=[self.mgr.tkey.name], known_hosts=None, server_host_key_algs=['ssh-rsa'], config=[self.mgr.ssh_config_fname])
                except OSError:
                    raise
